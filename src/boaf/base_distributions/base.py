...
class BaseDistribution(ABC):
    ...
    def add_data(
        self, data: NDArray[np.float64], weight: NDArray[np.float64] = None
    ) -> None:
    ...
    @abstractmethod
    def add_one(self, x: NDArray[np.float64], weight: np.float64) -> None:
    ...
    @abstractmethod
    def rem_one(self, x: NDArray[np.float64], weight: np.float64) -> None:
