...
class NIW(BaseDistribution):
    ...
    def __init__(self, mu, nu, kappa, sigma, data=None, weight=None) -> None:
...
class BLR(BaseDistribution):
    ...
    def __init__(self, m0, L0, a0, b0, data=None, weight=None) -> None:
    ...
    def add_one(self, data, weight=1):
        ...
        self.mu = np.linalg.solve(self.Lam, np.dot(self.L0, self.m0) + self.Xy)
        self.yy += weight * data[1] ** 2
        self.bn = self.b0 + 0.5 * (
            self.yy
            + self.m0.T.dot(self.L0).dot(self.m0)
            - self.mu.T.dot(self.Lam).dot(self.mu)
        )
    ...
    def posterior_predictive(self, Xt):
        ...
        Sig = (
            self.bn
            / self.an
            * (np.identity(Xt.shape[0]) + Xt.dot(np.linalg.inv(self.Lam)).dot(Xt.T))
        )
